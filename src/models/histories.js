...
exports.getDataHistoriesAsync = (data) => new Promise((resolve, reject) => {
    ...
    db.query(`select h.id,h.user_id,h.idCard,h.fullname,h.mobilephone,h.bookingCode,v.name as brand,v.price,v.rate,v.photo as photo,c.name as category,h.rentStartDate,h.rentEndDate,
    h.qty,h.prepayment,h.status_id,s.status,DATEDIFF(h.rentEndDate,h.rentStartDate)+1 as day,v.price*h.qty*(DATEDIFF(h.rentEndDate,h.rentStartDate)+1) as totalPayment,h.payment_id
    from histories h join users u on h.user_id = u.id 
    join vehicles v on h.vehicle_id = v.id join categories c on v.category_id = c.id 
    join status s on h.status_id = s.id 
...
exports.getDataHistoriesByUserIdAsync = (data,id) => new Promise((resolve, reject) => {
    ...
    const query =  db.query(`select h.id,h.user_id,h.idCard,h.fullname,h.mobilephone,h.bookingCode,v.name as brand,v.price,v.rate,v.photo as photo,c.name as category,h.rentStartDate,h.rentEndDate,
   h.qty,h.prepayment,h.status_id,s.status,DATEDIFF(h.rentEndDate,h.rentStartDate)+1 as day,v.price*h.qty*(DATEDIFF(h.rentEndDate,h.rentStartDate)+1) as totalPayment,h.payment_id
   from histories h join users u on h.user_id = u.id 
   join vehicles v on h.vehicle_id = v.id join categories c on v.category_id = c.id 
   join status s on h.status_id = s.id 
...
exports.getDataHistory = (id, cb) => {
    db.query(`select h.id,h.user_id,h.bookingCode,h.idCard,h.fullname,h.emailAddress,h.location,h.mobilePhone,v.name as brand,v.price,v.rate,concat('${APP_URL}/',v.photo) as photo,c.name as category,h.rentStartDate,h.rentEndDate,h.qty,h.prepayment,h.payment_type,s.status,DATEDIFF(h.rentEndDate,h.rentStartDate)+1 as day,v.price*h.qty*(DATEDIFF(h.rentEndDate,h.rentStartDate)+1) as totalPayment, h.bookingCode,h.paymentCode,h.createdAt
    from histories h join users u on h.user_id = u.id 
...
exports.getDataHistoryAsync = (id) => new Promise((resolve, reject) => {
    db.query(`select h.id,h.user_id,h.bookingCode,h.idCard,h.fullname,h.emailAddress,h.location,h.mobilePhone,v.name as brand,v.price,v.rate,v.photo as photo,c.name as category,h.rentStartDate,h.rentEndDate,h.qty,h.prepayment,h.payment_type,h.status_id,s.status,DATEDIFF(h.rentEndDate,h.rentStartDate)+1 as day,v.price*h.qty*(DATEDIFF(h.rentEndDate,h.rentStartDate)+1) as totalPayment,h.payment_id,h.createdAt
    from histories h join users u on h.user_id = u.id 
    join vehicles v on h.vehicle_id = v.id 
    join categories c on v.category_id = c.id
    join status s on h.status_id = s.id 
